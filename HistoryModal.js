          ...
          {!editingRecord ? (
            <>
              <div className="px-4 pt-3 pb-2 border-b border-slate-100 shrink-0">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <button type="button" onClick={closeHistoryModal} className="px-3 py-2 rounded-xl bg-slate-100 text-slate-700 text-sm font-semibold">閉じる</button>
                </div>
                    ...
                    {['day','week','month'].map((mode, idx) => (
                      <button
                        ...
                        className={`h-[42px] rounded-2xl text-sm font-bold border ${
                          historyMode === mode ? 'bg-slate-800 text-white border-slate-800' : 'bg-slate-100 text-slate-700 border-slate-200'
                        }`}
                      >
                        {['日','週','月'][idx]}
                      </button>
                    ...
                    {['all','1','2'].map((value, idx) => (
                      <button
                        ...
                        className={`h-[40px] rounded-2xl text-sm font-bold border ${
                          historyFilter === value ? 'bg-sky-500 text-white border-sky-500' : 'bg-white text-slate-700 border-slate-200'
                        }`}
                      >
                        {['すべて','①','②'][idx]}
                      </button>
                    ...
                    {groupedHistory.map((group) => {
                      ...
                      return (
                        <div key={group.key} className="grid gap-2">
                          <button type="button" onClick={() => toggleMonthDay(group.key)} className={`w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-left active:bg-slate-50 ${shadowSub}`}>
                            <div className="flex items-center justify-between gap-3">
                              <div className="min-w-0">
                                <div className="text-[18px] font-bold text-slate-800">{formatFullDate(group.date)}</div>
                                <div className="mt-1 text-xs text-slate-500">{group.count}件</div>
                              </div>
                              <div className="shrink-0 text-right">
                                <div className="text-base font-bold text-slate-800">{formatMoney(group.total)}</div>
                                <div className="mt-1 text-xs font-semibold text-slate-500">{opened ? '閉じる' : '開く'}</div>
                              </div>
                            </div>
                          </button>
                          {opened && (
                            <div className="pl-3 grid gap-2">
                              {group.records.map((record) => <HistoryRecordCard key={record.id} record={record} onClick={openEditRecord} />)}
                            </div>
                          )}
                        </div>
                      );
            ...
            <>
              <div className="px-4 pt-3 pb-2 border-b border-slate-100 shrink-0">
                <div className="w-12 h-1.5 rounded-full bg-slate-200 mx-auto mb-3"></div>
                <div className="flex items-center justify-between">
                  <button type="button" onClick={closeEditRecord} className="px-3 py-2 rounded-xl bg-slate-100 text-slate-700 text-sm font-semibold">戻る</button>
              ...
              <div className="flex-1 overflow-y-auto px-4 py-3">
                <div className="grid gap-3">
                  <div className={`rounded-2xl border border-slate-200 bg-white p-4 ${shadowSub}`}>
                    ...
                    <div className="mt-2 text-[18px] font-bold text-slate-800">{formatFullDate(editingRecord.乗務日 || editingRecord.乗車時刻)}</div>
                    <div className="mt-1 text-sm text-slate-500">{formatTime(editingRecord.乗車時刻)} → {formatTime(editingRecord.降車時刻)}</div>
                  </div>
                  ...
                  <div className={`rounded-2xl border border-slate-200 bg-white p-4 ${shadowSub}`}>
                    ...
                    <div className="mt-2 relative">
                      ...
                      <input
                        ...
                        onChange={(e) => setEditingRecord((prev) => ({ ...prev, 金額入力: e.target.value.replace(/[^\d]/g, '') }))}
                    ...
                    <div className="mt-3 grid grid-cols-2 gap-2">
                      {['1','2'].map((v, idx) => (
                        <button
                          ...
                          className={`h-[52px] rounded-2xl text-lg font-bold border ${editingRecord.区分入力 === v ? (v === '1' ? 'bg-sky-500 text-white border-sky-500' : 'bg-emerald-500 text-white border-emerald-500') : 'bg-slate-50 text-slate-700 border-slate-200'}`}
                        >
                          {['①','②'][idx]}
                        </button>
                      ))}
                    </div>
                    <div className="mt-2 text-xs text-slate-500">①＝現金・領収証なし　②＝カード/QR・領収証あり含む</div>
                  </div>
                    ...
                    <div className="mt-3 grid grid-cols-4 gap-2">
                      {[1,2,3,4].map((count) => (
                        <button
                          ...
                          className={`h-[46px] rounded-2xl text-lg font-bold border ${Number(editingRecord.人数入力) === count ? 'bg-sky-500 text-white border-sky-500' : 'bg-slate-50 text-slate-700 border-slate-200'}`}
                        >{count}</button>
                      ))}
                    </div>
                  </div>
                  ...
                  {[
                    ...
                    <div key={key} className={`rounded-2xl border border-slate-200 bg-white p-4 ${shadowSub}`}>
                      ...
                      <input
                        ...
                        onChange={(e) => setEditingRecord((prev) => ({ ...prev, [key]: e.target.value }))}
