      ...
      <div className="flex-1 min-h-0 flex flex-col overflow-hidden pt-4">
        ...
        <div className="pt-4 shrink-0">
          <div className="grid grid-cols-4 gap-4">
            {[1, 2, 3, 4].map((count) => {
              const active = selectedPassengers === count;
              return (
                <button
                  key={count}
                  type="button"
                  ...
                  style={{
                    background: active ? C.PASSENGER_ACTIVE : C.PASSENGER_INACTIVE,
                    borderColor: "#c9ced6",
                  }}
                >
                  {count}
        ...
        <div className="pt-4 flex-1 min-h-0">
          <div className="grid gap-3">
            <button
              ...
              className={`${C.smallButtonBase} h-[70px] text-[24px] font-extrabold text-slate-900 disabled:text-slate-500`}
              style={{ background: hasPassengerSelected ? C.PAYMENT_CASH : C.PAYMENT_DISABLED }}
            >
              現金
            </button>
            ...
            <button
              ...
              className={`${C.smallButtonBase} h-[70px] text-[24px] font-extrabold text-slate-900 disabled:text-slate-500`}
              style={{ background: hasPassengerSelected ? C.PAYMENT_CARD : C.PAYMENT_DISABLED }}
            >
              カード・QR
            </button>
            ...
            <button
              ...
              className={`${C.smallButtonBase} h-[70px] text-[24px] font-extrabold text-slate-900 disabled:text-slate-500`}
              style={{ background: hasPassengerSelected ? C.PAYMENT_RECEIPT : C.PAYMENT_DISABLED }}
            >
              領収証発行
            </button>
          </div>
