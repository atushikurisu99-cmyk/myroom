window.AppComponents = (() => {
      ...
      >
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="text-xl font-bold text-slate-800">{formatMoney(record.金額)}</div>
          ...
          <div className="shrink-0 text-right">
            <div
              className={`inline-flex min-w-[34px] justify-center rounded-full px-2.5 py-1 text-xs font-bold ${
                type === "1" ? "bg-sky-100 text-sky-700" : "bg-emerald-100 text-emerald-700"
              }`}
            >
              {type === "1" ? "①" : "②"}
            </div>
        ...
        <div className="mt-3 grid gap-1 text-sm text-slate-600">
          ...
          {record.備考 ? <div className="truncate text-xs text-slate-500">備考：{record.備考}</div> : null}
  ...
  function PreviewHistoryRows({ previewRecords }) {
    if (previewRecords.length === 0) {
      return <div className="px-4 py-4 text-sm text-slate-400">まだ履歴はありません</div>;
    }

    return previewRecords.map((record) => (
      <div key={record.id} className="px-4 py-3 border-b last:border-b-0 border-slate-100">
        <div className="flex items-start justify-between gap-3">
          <div className="text-[15px] font-normal text-slate-700">{formatMoney(record.金額)}</div>
          <div className="text-xs text-slate-500 shrink-0">{recordType(record) === "1" ? "①" : "②"}</div>
        </div>
        <div className="mt-1 text-xs text-slate-500">
          {formatTime(record.乗車時刻)} → {formatTime(record.降車時刻)}
        </div>
    ...
    return (
      <div className="shrink-0" style={translateStyle}>
        {showHandle && (
          ...
          >
            <div className="text-[13px] font-semibold text-slate-400">↓ 下へ</div>
          </div>
        )}
        ...
        <div style={{ height: `${BOTTOM_CARD_HEIGHT}px` }}>
          <div className={`${cardClass} h-full overflow-hidden flex flex-col`}>
            ...
            <div className="flex-1 min-h-0 overflow-y-auto">
              <button type="button" onClick={openHistoryModal} className="w-full text-left active:bg-slate-50">
        ...
        <div className="w-full rounded-t-[28px] bg-white shadow-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
          <div className="px-4 pt-3 pb-4">
            ...
            <div className="mt-4 overflow-hidden rounded-2xl border border-slate-200 bg-white">
              ...
              <button type="button" className="w-full px-4 py-4 text-left text-base font-semibold text-slate-800 border-b border-slate-100 active:bg-slate-50">設定</button>
              <button type="button" onClick={openHistoryModal} className="w-full px-4 py-4 text-left text-base font-semibold text-slate-800 active:bg-slate-50">履歴</button>
            </div>
          </div>
        </div>
      </div>
    ...
    return (
      <div className="absolute inset-0 z-40 bg-slate-900/40 flex items-center justify-center px-4">
        <div className="w-full rounded-[28px] bg-white shadow-2xl p-5">
          <div className="text-[34px] font-black text-slate-800 tracking-[-0.04em]">{formatMoney(amount)}</div>
          ...
          <div className="mt-5 flex justify-end">
            <button type="button" onClick={onCancel} className="min-w-[92px] h-[44px] rounded-2xl bg-slate-100 text-slate-700 text-sm font-bold">戻る</button>
          </div>
        </div>
      </div>
    );
  ...
  function ViaDialog({ pendingViaPlace, onCancel, onRecord }) {
    return (
      <div className="absolute inset-0 z-40 bg-slate-900/40 flex items-center justify-center px-4">
        <div className="w-full rounded-[28px] bg-white shadow-2xl p-5">
          <div className="text-[18px] font-bold text-slate-800">現在地を経由地として記録します</div>
          <div className="mt-3 text-sm text-slate-500 truncate">{pendingViaPlace || "未取得"}</div>
          <div className="mt-5 grid grid-cols-2 gap-3">
            <button type="button" onClick={onCancel} className="h-[48px] rounded-2xl bg-slate-100 text-slate-700 font-bold">戻る</button>
            <button type="button" onClick={onRecord} className="h-[48px] rounded-2xl bg-slate-800 text-white font-bold">記録</button>
          </div>
        </div>
      </div>
    );
